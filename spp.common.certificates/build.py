...
def main():
    ...
    parser.add_argument(
        '--ca-password',
        help='CA password. It will be requested safely, if not specified.',
        required=False
    )
    parser.add_argument(
        '--localhost-password',
        help='Localhost certificate password. It will be requested safely, if not specified.',
        required=False
    )
    args = parser.parse_args()

    ca_password: str = args.ca_password or getpass.getpass('CA password: ')
    localhost_password: str = args.localhost_password or getpass.getpass(
        'Localhost certificate password: '
    )

    generate_certificates(ca_password, localhost_password)

def generate_certificates(ca_password: str, localhost_password: str) -> None:
    generate_certificate('spp-ca', 'spp-ca', ca_password)
    generate_certificate('localhost', 'localhost', localhost_password, 'spp-ca/spp-ca.pfx')
